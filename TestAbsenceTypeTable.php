...
function AbsenceTypeTableTest_Create_004() {
    ...
    if ($absenceType != NULL AND $absenceType[ABS_TYPE_ID] <> NULL) {
        ...
        if (($absenceTypeFromDB[ABS_TYPE_ID] == $absenceType[ABS_TYPE_ID]) AND ( strcmp($absenceTypeFromDB[ABS_TYPE_NAME], "Sick Leave") == 0) AND ( $absenceTypeFromDB[ABS_TYPE_USES_LEAVE] == FALSE) AND ( $absenceTypeFromDB[ABS_TYPE_CAN_BE_DENIED] == TRUE)) {
            $testResult = "Pass";
        }
    }
    rowTestTableOutput(__FUNCTION__, $testTitle, $testResult, $testError);
        ...
        foreach ($absenceTypes as $absenceType) {
            if ($absenceType <> NULL) {
                if ($absenceType[ABS_TYPE_NAME] == "Sick Leave") {
                    if (($absenceType[ABS_TYPE_USES_LEAVE] <> FALSE ) OR ( $absenceType[ABS_TYPE_CAN_BE_DENIED] <> FALSE)) {
                        $testError = "Sick Leave fields incorrect";
                        $testResult = "Fail";
                    }
                } else if ($absenceType[ABS_TYPE_NAME] == "Annual Leave") {
                    if (($absenceType[ABS_TYPE_USES_LEAVE] <> TRUE) OR ( $absenceType[ABS_TYPE_CAN_BE_DENIED] <> TRUE )) {
                        $testError = "Annual Leave fields incorrect";
                        $testResult = "Fail";
                    }
                } else if ($absenceType[ABS_TYPE_NAME] == "Training") {
                    if (($absenceType[ABS_TYPE_USES_LEAVE] <> FALSE ) AND ( $absenceType[ABS_TYPE_CAN_BE_DENIED] <> TRUE )) {
                        $testError = "Training fields incorrect";
                        $testResult = "Fail";
                    }
                } else if ($absenceType[ABS_TYPE_NAME] == "Emergency Leave") {
                    if (($absenceType[ABS_TYPE_USES_LEAVE] <> TRUE ) OR ( $absenceType[ABS_TYPE_CAN_BE_DENIED] <> FALSE)) {
...
function testAbsenceTypeTableCreate() {
    CreateNewDatabase(true);  //Start our testing with an empty database.
    //--------------------------------------------------------------
    // Run the tests
    //--------------------------------------------------------------
